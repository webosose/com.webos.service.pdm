...
then
    ...
    DEVICE_SET_ID=$(luna-send -n 1 -f luna://com.webos.service.sessionmanager/getSessionList '{}' | grep deviceSetId | awk '{print $2}' | tr -d ',"' | tail -1)
    if [ "$DEVICE_SET_ID" == "RSE-R" ]
    then
...
then
    ...
    if [ "$DEVICE_SET_ID1" == "RSE-R" ]
    then
        SESSION_ID=$(luna-send -n 1 -f luna://com.webos.service.sessionmanager/getSessionList '{}' | grep sessionId | awk '{print $2}' | tr -d ',"' | head -1)
    elif [ "$DEVICE_SET_ID2" == "RSE-R" ]
    then
...
CONTAINER_R=$(docker inspect "$SESSION_ID" | grep MergedDir | awk '{print $2}' | tr -d '",')
echo "RSE-R: $CONTAINER_R"
mkdir $CONTAINER_R/tmp/usb_rse_right
mkdir $CONTAINER_R/tmp/usb_rse_right/$1
DRIVE="/dev/"$1
mount $DRIVE $CONTAINER_R/usb_rse_right/$1
